...
import {normalizeURL, getName} from './util'
import {
  Container,
  InputSection,
  InputSectionRow2,
...
export function YesContact({
    ...
    <Container>
      <InputSection>
        ...
        <InputSectionRow2>
          ...
          <PostButton
            onClick={publicKey ? sendPrivateMessage : establishNostrKey}
            disabled={!editable}
          >
            {publicKey ? (editable ? 'Send' : 'Sending') : 'Get Key'}
          </PostButton>
        </InputSectionRow2>
      </InputSection>
        ...
        <Info>
          ...
          <p>
            ...
            <a
              ...
              target="_blank"
              href="https://github.com/okjodom/yescontact"
            >
              YesContact
            </a>
            .
  ...
  async function sendPrivateMessage() {
    ...
    let ciphertext = await nip04.encrypt(senderPrivateKey, receiverPublicKey, message)
    ...
    if (senderPrivateKey) {
      ...
      try {
        ...
        showNotice(
          `Failed to sign event: ${err.message}`
        )
        setEditable(true)
        return
      }
    ...
    const publishTimeout = setTimeout(() => {
      showNotice(
        `Failed to publish message ${event.id.slice(0, 5)}… to any relay. Try again.`
      )
      setEditable(true)
    }, 8000)
    ...
    connections.current.forEach(conn => {
      ...
      pub.on('ok', () => {
        ...
        showNotice(`Message ${event.id.slice(0, 5)}… successfully published to ${conn.url}.`)
        setMessage('')
        setEditable(true)
      })
    })
